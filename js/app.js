...
let all = document.querySelectorAll(".parent .container .min_container .boxes");
aside.forEach((li) => {
...
function chick() {
  ...
  if (localStorage.background) {
    dataBg = JSON.parse(localStorage.background);

    let parent = document.querySelector(".parent");
    parent.style.cssText = `background-image: url(${dataBg})`;
  }
...
function addTaskToArray(arr, text, id, star) {
  let task = { id: id, title: text, complete: star };
  arr.push(task);
}
...
function deleteText(where, arr, spaceValue, complete, text) {
